...
export const ClientAvatarUpload = ({ 
  ...
  const getInitials = () => {
    ...
    return clientName.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase();
  };
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    try {
      ...
      // Atualizar cliente com novo avatar
      ...
      if (currentAvatarUrl && !currentAvatarUrl.startsWith('http')) {
        const oldPath = currentAvatarUrl.includes('/') 
          ...
          : currentAvatarUrl;
        if (oldPath) {
          invalidateAvatarCache(oldPath, 'client-avatars');
        }
      }
      invalidateAvatarCache(filePath, 'client-avatars');
    ...
    try {
      ...
      if (!currentAvatarUrl.startsWith('http')) {
        ...
        if (path) {
          ...
          invalidateAvatarCache(path, 'client-avatars');
        }
      }
      ...
      toast({
        title: 'Foto removida',
        description: 'A foto do cliente foi removida com sucesso.',
      });
    } catch (error: any) {
    ...
    <div className="flex flex-col items-center space-y-3">
      <div className="relative">
        <Avatar className={`${sizeClasses[size]} border-4 border-background shadow-lg`}>
          <AvatarImage src={getAvatarUrl(currentAvatarUrl) || undefined} alt={clientName} />
